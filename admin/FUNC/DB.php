...
class DB {
    ...
    static public $obj = array ( "sql_database"   => ""         ,
	"sql_user"       => ""     ,
                       "sql_pass"       => ""         ,
                       "sql_host"       => "",
                       "sql_port"       => 3306         ,
     ...
     static private $query_id      = "";
     static private $connection_id = "";
     static private $query_count   = 0;
     static private $record_row    = array();
     static private $return_die    = 0;
     static private $error         = "";
     static private $failed        = 0;
   ...
   static public function connect() {
		define('SQL_PREFIX', '');
		self::$connection_id = mysqli_connect( self::$obj['sql_host'] ,
											  self::$obj['sql_user'] ,
											  self::$obj['sql_pass'] ,
											  NULL ,
											  self::$obj['sql_port'] ,
        ...
        {    
             self::$fatal_error('ERROR: Link-ID == false, connect failed');
             return false;
        }  
   
        if ( !mysqli_select_db(self::$connection_id, self::$obj['sql_database']) )
        {
             self::$fatal_error("ERROR: Cannot find database ".self::$obj['sql_database']);
             return false;            
        }
    }
   ...
   static public function query($the_query, $bypass=0) {
        ...
        self::$query_id = mysqli_query(self::$connection_id, $the_query);
      
        if (!self::$query_id )
        {	
            self::$fatal_error("mySQL query error: $the_query");
        }
        ...
        self::$obj['cached_queries'][] = $the_query;
        
        return self::$query_id;
    }

    static public function fetch_row($query_id = "") {
